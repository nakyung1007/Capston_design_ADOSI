...
# # 모델과 스케일러 불러오기
# model_path = '/Users/chonakyung/modelmodel/DDOS_model (1).joblib'
# try:
#     model = joblib.load(model_path)
# except AttributeError as e:
#     print(f"Error loading model: {e}")

# scaler = joblib.load('/Users/chonakyung/modelmodel/scaler (3).joblib')
...
class Model:
    ...
    def predict(self, sample_data):
        #loaded_scaler = load('/Users/chonakyung/modelmodel/scaler (3).joblib')
        new_data = self.scaler.transform(sample_data)
        predicted_labels = self.RF.predict(new_data)
        return predicted_labels
    
# 모델과 스케일러 불러오기
model = '/Users/chonakyung/modelmodel/DDOS_model (1).joblib'
# try:
#     model = joblib.load(model_path)
# except AttributeError as e:
#     print(f"Error loading model: {e}")
...
def extract_flags(tcp_layer):
    ...
    # }
    flags = {
        'FIN': int(tcp_layer.flags & 0x01),
        'SYN': int(tcp_layer.flags & 0x02),
        'RST': int(tcp_layer.flags & 0x04),
        'PSH': int(tcp_layer.flags & 0x08),
        'ACK': int(tcp_layer.flags & 0x10),
        'URG': int(tcp_layer.flags & 0x20),
        'ECE': int(tcp_layer.flags & 0x40),
    }
    return flags
...
def packet_callback(packet):
    ...
    try:
        if IP in packet:
            if packet[IP].dst == ip_address and (TCP in packet or UDP in packet):
                ...
                if 'last_time' not in globals():
                    ...
                    return  # 첫 패킷은 IAT 계산을 건너뜁니다.

                iat = float(packet.time) - last_time
                if iat > 0:
                    iat_times.append(iat)
                    flow_packets_s = 1 / iat
                    flow_bytes_s = len(packet) / iat

                last_time = float(packet.time)

                packet_info = {
                    'Flow Duration': int((float(packet.time) - last_time) * 1e9),
                    'Total Fwd Packets': 1 if sport == port else 0,
                    'Total Backward Packets': 1 if dport == port else 0,
                    ...
                    'URG Flag Count': flags.get('URG', 0),
                    'ECE Flag Count': flags.get('ECE', 0),
                    'Fwd Packets Length Total': len(packet) if sport == port else 0,
                    'Bwd Packets Length Total': len(packet) if dport == port else 0,
                    'Flow IAT Mean': np.mean(iat_times) if iat_times else 0,
                    'Flow IAT Std': np.std(iat_times) if iat_times else 0,
                    'Idle Mean': np.mean(iat_times) if iat_times else 0,
                    'Protocol': protocol
                }
                ...
                # 100개 패킷을 캡처하면 예측 수행
                if len(packet_infos) % batch_size == 0:
                    predict_and_print(packet_infos[-batch_size:])
                    # 예측 후 리스트 초기화
                    packet_infos = []
                    iat_times = []
    except Exception as e:
...
def predict_and_print(packet_batch):
    features = [
        'Flow Duration', 'Total Fwd Packets', 'Total Backward Packets', 'Flow Packets/s', 
        'Flow Bytes/s', 'Avg Packet Size', 'FIN Flag Count', 'SYN Flag Count', 
        'RST Flag Count', 'PSH Flag Count', 'ACK Flag Count', 'URG Flag Count', 
    ...
    sample_data = pd.DataFrame(packet_batch, columns=features)
    sample_data_scaled = scaler.transform(sample_data)
    predictions = model.predict(sample_data_scaled)
...
# def capture_and_predict_packets(interface, ip_address, port):
#     capture = pyshark.LiveCapture(interface=interface, bpf_filter=f"tcp and dst host {ip_address} and dst port {port}")
#     capture.apply_on_packets(packet_callback)
#     return predict_and_print(packet_infos[-batch_size:])
...
@app.route('/analyze_packets', methods=['GET'])
def analyze_packets():
    sniff(prn=packet_callback, filter=f"tcp and dst host {ip_address} and dst port {port}", store=0, iface=interface)
    return "Packet analysis started. Check console for predictions."
