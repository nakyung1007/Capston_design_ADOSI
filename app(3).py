...
# #     logging.info(f"Request from {client_ip}: {request.method} {request.url}")
...
# # # 모델과 스케일러 불러오기
# # # model = load('/Users/chonakyung/Desktop/capstone/packet/Classification_model_vol_fin.joblib')
...
# # def capture_and_predict_packets(interface, ip_address, port, capture_count=100):
# #     while True:
# #         try:
...
# #                      # 프로토콜 매핑
# #                     protocol = 6 if packet.transport_layer == 'TCP' else 17 if packet.transport_layer == 'UDP' else 0
...
# #             # Feature names를 제거하고 예측
# #             # sample_data_scaled = pd.DataFrame(sample_data_scaled).values
# #             sample_data_scaled_df = pd.DataFrame(sample_data_scaled, columns=scaler.feature_names_in_)
# #             predictions = model.predict(sample_data_scaled_df.values)
...
# #     # 커스텀 RequestHandler를 사용하여 비정상적인 요청을 허용
# #     class CustomRequestHandler(WSGIRequestHandler):
# #         def handle_one_request(self):
# #             try:
# #                 super().handle_one_request()
# #             except Exception as e:
# #                 self.log_error(f"Exception occurred: {e}")
        ...
        try:
            ...
            for packet in capture.sniff_continuously(packet_count=capture_count):
                if 'IP' in packet:
                    ...
                    # 프로토콜 매핑
                    protocol = 6 if packet.transport_layer == 'TCP' else 17 if packet.transport_layer == 'UDP' else 1
            ...
            sample_data = pd.DataFrame(packet_infos, columns=features)
            logging.info(f"Sample data before scaling: {sample_data.head()}")
            sample_data = sample_data.reindex(columns=scaler.feature_names_in_, fill_value=0)
            logging.info(f"Sample data after reindexing: {sample_data.head()}")
            sample_data_scaled = scaler.transform(sample_data)
            logging.info(f"Sample data after scaling: {sample_data_scaled[:5]}")
...
# def capture_and_predict_packets(interface, ip_address, port, capture_duration=10):
#     while True:
#         try:
#             capture_filter = f'ip src {ip_address}'
#             capture = pyshark.LiveCapture(interface=interface, bpf_filter=capture_filter)
...
#                 packet = capture.sniff_continuously(packet_count=1)
#                 packet = next(packet)
#                 if 'IP' in packet:
#                     ip_layer = packet.ip
#                     tcp_layer = packet.tcp if hasattr(packet, 'tcp') else None
...
#                         'ECE Flag Count': flags.get('ECE', 0),
#                         'Fwd Packets Length Total': int(packet.length) if tcp_layer and tcp_layer.srcport == port else 0,
#                         'Bwd Packets Length Total': int(packet.length) if tcp_layer and tcp_layer.dstport == port else 1,
#                         'Flow IAT Mean': np.mean(iat_times) if iat_times else 1,
#                         'Flow IAT Std': np.std(iat_times) if iat_times else 1,
#                         'Idle Mean': np.mean(iat_times) if iat_times else 0,  # Assuming idle time is same as IAT for now
#                         'Protocol': protocol
#                     }
#                     packet_infos.append(packet_info)
...
#         except Exception as e:
#             print(f"Error during packet capture and prediction: {e}")
#             logging.error(f"Error during packet capture and prediction: {e}")
#             time.sleep(5)
